...
{
    // Convert cnyfund:// to cnyfund:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cnyfund://"))
    {
        uri.replace(0, 12, "cnyfund:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CNY.lnk";
}
...
{
    return GetAutostartDir() / "cnyfund.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CNYF-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  cnyfunds-qt [" + tr("command-line options") + "]                     " + "\n";
